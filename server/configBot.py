...
def rand_ant_prompt(n_colors, prompt = ""):
	return {"instructions": default_instructions,
		...
		"template": default_ant_template}
	
default_plant_template = """{
   "defaultRule": {"turn": "N"},
   "rules": {
	"0": {"turn": "right", "move": ""},
	"1": {"turn": "left", "move": ""}
...
def rand_plant_prompt(n_colors, prompt = ""):
	return {"instructions": default_instructions,
		"prompt": f"""Modfiy this default configuration for a Langton's Plant. Use the available actions to determine what a plant should do when on that color square.

		"""
		"template": default_plant_template}
